...
CURL调用发布
```
curl -X POST -H "Content-Type: application/json" -X POST  --user ops:wwh85650550   -H "Accept: application/json"  http://127.0.0.1:8000/api/deployJobViewSet/ -d "{\"project_version\":\"http://127.0.0.1:8000/api/projectversionViewSet/6/\",\"job_name\":\"curl\"}" 
```